...
class GroupDROLoss(nn.Module):
    ...
    def forward(self, losses: torch.Tensor, group_names: List[str]):
        ...
        batch_group_weights = torch.exp(torch.stack([self.weights[self.g2idx[g]] for g in group_names])).to(device)
...
def train(cfg: Config):
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        with torch.no_grad():
            ...
            auroc_metric.update(
            torch.tensor(v_probs, dtype=torch.float32),
            torch.tensor(v_labels, dtype=torch.long))
            auprc_metric.update(torch.tensor(v_probs, dtype=torch.float32),
            torch.tensor(v_labels, dtype=torch.long))
